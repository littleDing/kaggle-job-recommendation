...
jid_title = load_jid_title()
uid_tags  = load_uid_tags()
...
with open(wd + "users.tsv", "r") as infile:
    ...
    with open("popular_jobs.csv", "w") as outfile:
        ...
        for line in reader:
            ...
            if True : #len(top_jobs) < 150:
                top_jobs += top_state_jobs[int(WindowId)][State]
            top_jobs = [int(x[0]) for x in top_jobs ]
            if True : #len(top_jobs) > 150:
				tmp = []
				uid = int(UserId)
				for jid in top_jobs :
					title = jid_title[jid]
					if 0 != len(uid_tags[uid] & set(title.split()) ) :
						tmp.append(jid)
				top_jobs = tmp
            top_jobs = top_jobs[0:150]
            outfile.write(str(UserId) + "," + " ".join([ str(jid) for jid in top_jobs]) + "\n")

...
wd = DATA_DIR # The directory that the data files are in
...
with open(wd + "users.tsv", "r") as infile:
    ...
    quoting=csv.QUOTE_NONE, quotechar="")
    reader.next() # burn the header
    with open(DATA_DIR + "popular_jobs_cdmm.csv", "w") as outfile:
        ...
        for line in reader:
            ...
            outfile.write(str(UserId) + "," + " ".join([str(x[0]) for x in top_jobs]) + "\n")
            haha.inc()
